...
class FamilyNormalizer implements NormalizerInterface
{
    ...
    public function normalize($family, $format = null, array $context = [])
    {
        ...
        foreach($associations as $association) {
			...
			//file_put_contents(dirname(__FILE__)."/file.json", json_encode($normalizedFamily)."=>". json_encode($family->getAttributes()));
        foreach ($family->getAttributes() as $attr) {
            $normalizedFamily['attribute_groups'][$attr->getGroup()->getCode()]['code'] = $attr->getGroup()->getCode();
